...
function setUiUrl(results, scanId, scanConfigId) {
    ...
    for(let i=0; i<results.data.data.length; i++){
        ...
        if(results.data.data[i].insight_ui_url){
            insightUiUrl = results.data.data[i].insight_ui_url;
            insightUiUrl = insightUiUrl.split("#");
            insightUiUrl = insightUiUrl[0];
            break;
        } else {
            continue;
        }
    ...
    return `${insightUiUrl}#/apps/${appId}/configuration/${scanConfigId}/scan/${scanId}`;
}
    ...
    try {
        ...
        if (!waitScanComplete) {
            ...
            if(success) {
                ...
                core.setOutput(OUTPUT_SCAN_FINDINGS, JSON.stringify({vulnerabilities: result.vulnerabilities, scanLink: result.scanLink}, null, 2));
