...
function setUiUrl(results, scanId, scanConfigId) {
    ...
    if(!results.data.data[0].insight_ui_url){
        log.debug("insight_ui_url not found.");
        return;
    }
...
(() => {
const core = __nccwpck_require__(450);
const InsightAppSecClient = __nccwpck_require__(747);
const ScanTools = __nccwpck_require__(189);
    ...
    try {
        ...
        if (!waitScanComplete) {
            ...
            if(success) {
                ...
                core.setOutput(OUTPUT_SCAN_FINDINGS, JSON.stringify({results}, null, 2));
